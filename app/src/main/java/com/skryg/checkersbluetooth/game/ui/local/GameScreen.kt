...
@Composable
fun LocalGameButtons(modifier:Modifier=Modifier, onClickDraw: ()->Unit, onClickResign: ()-> Unit,rotated: Boolean = false){
...
fun LocalGameScreen(viewModel: LocalGameViewModel = viewModel(factory = AppViewModelProvider.Factory)){
    Column(Modifier.fillMaxSize()){
        LocalGameButtons(Modifier,{}, {}, rotated=true)
        val state = remember { mutableStateOf(UiState()) }
        Board(modifier = Modifier.weight(1f),state = state)
        LocalGameButtons(Modifier,{},{},rotated = false)
    }
}
